...
| Release verification | Add `-VerifyReleaseArtifacts` to `-Check` to download a few GitHub release assets and compare them with their published SHA-256 files. Each run checks the slice after the one the committed report recorded, so every asset gets its turn however often you run it. A mismatch fails the run, and an asset that can't be downloaded is only a warning. Without the switch, release evidence stays metadata-only. |
