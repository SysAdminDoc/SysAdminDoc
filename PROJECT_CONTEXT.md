...
- `scripts/write-profile-sync-summary.ps1` renders public-safe GitHub Actions job summaries and annotations from `reports/profile-sync-report.json`, including aggregate catalog-accounting, README-density and portfolio-only preview, generated PR dry-run evidence, project-license, fork-parent, profile release/tag, userscript install trust, repository-setting, community-health, and stale-project/archive-review counts, with hard/soft size-budget checks before writing `GITHUB_STEP_SUMMARY`.
- `scripts/open-generated-profile-pr.ps1` centralizes generated profile PR creation for profile-sync and profile-assets refresh workflows, including branch creation, no-change guards, explicit generated-artifact staging, PR creation, validation dispatch, job-summary links, and a side-effect-free `-DryRun` mode for generated PR delivery rehearsal.
...
- `requiredCheckReadiness.prDeliveryTransition` reports `blocked` with 5 checklist rows: 2 ready, 2 blocked, and 1 needing live validation before required-check enforcement. `generatedPrDryRunEvidence` records successful hosted run `27084524165`, which uploaded a report artifact and reached the preview helper without creating a branch, commit, push, pull request, or validation dispatch.
- `repositorySettings.security.codeScanning` reports `status=not-applicable`, `recommendation=not-applicable-powershell-only`, inspected language `PowerShell`, no CodeQL workflow, Scorecard SARIF upload present, and active controls for secret scanning, push protection, PSScriptAnalyzer, actionlint, zizmor, and OpenSSF Scorecard SARIF.
...
- `profileReleaseConsistency` compares planning version `v4.9.101` against latest GitHub release `v3.0.0`, records the missing `v4.9.101` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
