...
- `profileReleaseConsistency` compares planning version `v4.9.59` against latest GitHub release `v3.0.0`, records the missing `v4.9.59` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
1. Reconcile stale roadmap duplicate rows for already shipped profile validation and issue-form work.
2. Audit branch-protection/ruleset status-check enforcement constraints.
