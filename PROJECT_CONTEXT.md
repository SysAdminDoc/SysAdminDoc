...
- `profileReleaseConsistency` compares planning version `v4.9.71` against latest GitHub release `v3.0.0`, records the missing `v4.9.71` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
2. Add a stale-project and archive-review report derived from `pushedAt`, latest releases, and suppression reasons.
3. Add a markdownlint check for generated README-safe Markdown rules if the whitespace-only EditorConfig coverage is not enough.
