...
- `profileReleaseConsistency` compares planning version `v4.9.88` against latest GitHub release `v3.0.0`, records the missing `v4.9.88` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
- `readmeDensity` records generated README line/details/table/category counts plus repo-only and low-signal row counts, routing recommendation, portfolio-only candidate counts, per-category soft-limit overflow counts, deterministic candidate selection policy, concrete portfolio-only candidate rows, and a report-only preview showing that the current 11 candidates would reduce README rows from 177 to 166, reduce Python from 41 to 30 rows, and clear the current category soft-limit warning without mutating catalog, README, or feed output.
...
- `schemaValidation.passed=true`; the normalized catalog, generated projects feed, and sync report validate against `schemas/profile-catalog.v1.json`, `schemas/profile-projects.v1.json`, and `schemas/profile-sync-report.v1.json`, including the required `releaseTrust` object, `userscriptInstallTrust`, `catalogFeedAccounting`, `staleProjectReview`, `readmeDensity.portfolioOnlyCandidates`, `readmeDensity.portfolioOnlyPreview`, `requiredCheckReadiness.workflowCoverage`, and `requiredCheckReadiness.prDeliveryTransition` report sections, `docVersionConsistency.changelogHeadingValidation`, and the report `schemaValidation.report` result.
