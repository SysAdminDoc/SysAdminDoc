...
- `scripts/write-profile-sync-summary.ps1` renders public-safe GitHub Actions job summaries and annotations from `reports/profile-sync-report.json`, including aggregate catalog-accounting, README-density and portfolio-only preview, project-license, fork-parent, profile release/tag, userscript install trust, repository-setting, community-health, and stale-project/archive-review counts.
...
- `requiredCheckReadiness.prDeliveryTransition` reports `blocked` with 5 checklist rows: 2 ready, 2 blocked, and 1 needing live validation before required-check enforcement.
...
- `profileReleaseConsistency` compares planning version `v4.9.90` against latest GitHub release `v3.0.0`, records the missing `v4.9.90` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
- `schemaValidation.passed=true`; the normalized catalog, generated projects feed, and sync report validate against `schemas/profile-catalog.v1.json`, `schemas/profile-projects.v1.json`, and `schemas/profile-sync-report.v1.json`, including deterministic aggregate report arrays, optional catalog `readmeReviewNote`, required `releaseTrust` object, `userscriptInstallTrust`, `catalogFeedAccounting`, `staleProjectReview`, `readmeDensity.portfolioOnlyCandidates.catalogReviewNote`, `readmeDensity.portfolioOnlyPreview`, `requiredCheckReadiness.workflowCoverage`, and `requiredCheckReadiness.prDeliveryTransition` report sections, `docVersionConsistency.changelogHeadingValidation`, and the report `schemaValidation.report` result.
