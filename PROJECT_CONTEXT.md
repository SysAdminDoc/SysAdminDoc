...
- `profileReleaseConsistency` compares planning version `v4.9.65` against latest GitHub release `v3.0.0`, records the missing `v4.9.65` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
2. Add `schemas/**` to the offline Tests workflow path filters.
