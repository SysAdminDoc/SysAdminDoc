...
- `profileReleaseConsistency` compares planning version `v4.9.74` against latest GitHub release `v3.0.0`, records the missing `v4.9.74` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
1. Triage failing checks on Dependabot PR #7 or document why that branch should wait.
