...
- `profileReleaseConsistency` compares planning version `v4.9.68` against latest GitHub release `v3.0.0`, records the missing `v4.9.68` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
2. Refresh stale catalog field names in completed-work docs.
3. Add `.editorconfig` pinning for LF, final newline, and trailing-whitespace trimming.
