...
- `docs/decisions/2026-06-07-portfolio-only-demotion-review.md` approves the current 11 reviewed README density candidates for a later staged portfolio-only catalog mutation without changing generated output in the decision pass.
...
- `projectLicenseMetadata` checks 177 visitor-facing rows, detects 174 project licenses, records 3 missing-license rows, records 9 non-standard GitHub `other` rows, and aggregates 156 MIT, 3 GPL-3.0, 2 Apache-2.0, 1 AGPL-3.0, 1 BSD-3-Clause, 1 LGPL-3.0, 1 MPL-2.0, and 9 NOASSERTION rows with deterministic report-row ordering.
...
- `profileReleaseConsistency` compares planning version `v4.9.94` against latest GitHub release `v3.0.0`, records the missing `v4.9.94` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
- `readmeSizeBudget` reports the generated README at 65,901 bytes against a 98,304-byte soft cap, with `overSoftLimit=false` and no warning.
- `readmeDensity` records generated README line/details/table/category counts plus repo-only and low-signal row counts, routing recommendation, portfolio-only candidate counts, per-category soft-limit overflow counts, deterministic candidate selection policy, concrete portfolio-only candidate rows with catalog review notes, and a report-only preview showing that the current 11 candidates would reduce README rows from 177 to 166, reduce Python from 41 to 30 rows, and clear the current category soft-limit warning without mutating catalog, README, or feed output.
