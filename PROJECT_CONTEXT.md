...
- `profileReleaseConsistency` compares planning version `v4.9.76` against latest GitHub release `v3.0.0`, records the missing `v4.9.76` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
2. Inspect generated README category density and whether low-signal rows should move toward portfolio-only browsing.
3. Record code-scanning posture so missing CodeQL is not chased for a PowerShell-only profile repo.
