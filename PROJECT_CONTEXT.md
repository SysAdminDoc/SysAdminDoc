...
- `profileReleaseConsistency` compares planning version `v4.9.69` against latest GitHub release `v3.0.0`, records the missing `v4.9.69` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
2. Add `.editorconfig` pinning for LF, final newline, and trailing-whitespace trimming.
3. Record a decision note on the retained third-party render hosts.
