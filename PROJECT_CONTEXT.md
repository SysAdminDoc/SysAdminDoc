...
- `profileReleaseConsistency` compares planning version `v4.9.87` against latest GitHub release `v3.0.0`, records the missing `v4.9.87` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
1. Add a PR-delivery dry-run helper once branch-based delivery is approved.
2. Add candidate-row override notes to the catalog now the preview mode is in place.
3. Add deterministic ordering for report aggregate arrays that still churn across live metadata snapshots.
