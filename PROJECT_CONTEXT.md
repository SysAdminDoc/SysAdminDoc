...
- `scripts/write-profile-sync-summary.ps1` renders public-safe GitHub Actions job summaries and annotations from `reports/profile-sync-report.json`, including aggregate catalog-accounting, README-density, project-license, fork-parent, profile release/tag, userscript install trust, repository-setting, community-health, and stale-project/archive-review counts.
...
- `profileReleaseConsistency` compares planning version `v4.9.86` against latest GitHub release `v3.0.0`, records the missing `v4.9.86` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
- `readmeDensity` records generated README line/details/table/category counts plus repo-only and low-signal row counts, routing recommendation, portfolio-only candidate counts, per-category soft-limit overflow counts, deterministic candidate selection policy, and concrete portfolio-only candidate rows; density warnings are informational and feed the next portfolio-only review pass.
...
- `schemaValidation.passed=true`; the normalized catalog, generated projects feed, and sync report validate against `schemas/profile-catalog.v1.json`, `schemas/profile-projects.v1.json`, and `schemas/profile-sync-report.v1.json`, including the required `releaseTrust` object, `userscriptInstallTrust`, `catalogFeedAccounting`, `staleProjectReview`, `readmeDensity.portfolioOnlyCandidates`, `requiredCheckReadiness.workflowCoverage`, and `requiredCheckReadiness.prDeliveryTransition` report sections, `docVersionConsistency.changelogHeadingValidation`, and the report `schemaValidation.report` result.
...
- Profile sync summaries include a short README portfolio-only candidate sample.
...
1. Add a portfolio-only catalog review preview mode before changing generated README rows.
2. Add a PR-delivery dry-run helper once branch-based delivery is approved.
3. Add candidate-row override notes to the catalog only after the preview mode is in place.
