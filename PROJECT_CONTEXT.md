...
- `profileReleaseConsistency` compares planning version `v4.9.73` against latest GitHub release `v3.0.0`, records the missing `v4.9.73` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
2. Continue reconciling stale duplicate roadmap rows against shipped evidence before taking new feature work.
3. Add a lightweight report for npm/Node validation-tool drift if the markdownlint lockfile starts to need broader toolchain management.
