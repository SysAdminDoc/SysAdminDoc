...
1. Record code-scanning posture so missing CodeQL is not chased for a PowerShell-only profile repo.
2. Audit downstream portfolio compatibility before changing any feed shape.
3. Decide whether density-warning rows should move to portfolio-only browsing, using `readmeDensity` evidence.
