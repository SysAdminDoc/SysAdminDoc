...
- `.github/workflows/workflow-security.yml` creates all-PR and merge-queue checks, runs checksum-verified `actionlint` against workflow YAML, and runs hash-checked `zizmor` against workflows plus future local action definitions under `.github`.
...
- `profileReleaseConsistency` compares planning version `v4.9.64` against latest GitHub release `v3.0.0`, records the missing `v4.9.64` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
- `schemaValidation.passed=true`; the normalized catalog, generated projects feed, and sync report validate against `schemas/profile-catalog.v1.json`, `schemas/profile-projects.v1.json`, and `schemas/profile-sync-report.v1.json`, including the required `releaseTrust` object, `userscriptInstallTrust` and `catalogFeedAccounting` report sections, `docVersionConsistency.changelogHeadingValidation`, and the report `schemaValidation.report` result.
...
2. Stagger the `assets-refresh` and `workflow-security` Wednesday schedules.
