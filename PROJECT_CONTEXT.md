...
- Full live link validation has 0 fatal failures and 0 warnings in the latest v4.9.14 sync report.
...
As of v4.9.0, the sync script retries GraphQL metadata and falls back to GitHub REST metadata when GraphQL returns transient 502 errors. The README includes generated Start Here and Catalog Snapshot sections, action columns for Featured/Currently Building rows, category anchors, category previews, and committed local SVG metric panels. The generated project feed includes `primaryAction`, `hasDownload`, `hasLiveDemo`, `hasDirectInstall`, `releaseAssetKinds`, `releaseAssetNames`, and `releaseAssetInspected`.
...
3. Publish or repoint the advertised JSON Schema URLs, then validate the feed against them.
4. Add doc version/date consistency checks to `-Check` and CI.
5. Harden `setup.ps1` with check-only diagnostics, transcript logging, and inspect-before-run documentation.
6. Add quarterly archive/retirement reporting.
