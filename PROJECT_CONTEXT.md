...
- `profileReleaseConsistency` compares planning version `v4.9.77` against latest GitHub release `v3.0.0`, records the missing `v4.9.77` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
1. Inspect generated README category density and whether low-signal rows should move toward portfolio-only browsing.
2. Record code-scanning posture so missing CodeQL is not chased for a PowerShell-only profile repo.
3. Audit downstream portfolio compatibility before changing any feed shape.
