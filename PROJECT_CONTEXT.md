...
- `.github/workflows/tests.yml` runs offline Pester, pinned PSScriptAnalyzer, and a Windows PowerShell setup smoke check on generator/setup changes.
...
- `profileAssetsInSync=true`; six committed local SVG assets under `assets/profile/` match generated output.
...
- `linkValidationSummary` reports 185 URL targets checked with throttle 16 in 9148 ms and 0 warning host groups.
...
- `schemaValidation.passed=true`; the normalized catalog and generated projects feed validate against `schemas/profile-catalog.v1.json` and `schemas/profile-projects.v1.json`, including the required `releaseTrust` object.
...
- Workflow security now runs checksum-verified `actionlint` 1.7.12 before `zizmor`.
...
1. Add a strict JSON Schema contract for `reports/profile-sync-report.json`.
2. Promote `profile-sync-report.json` to a versioned schema contract.
3. Pin CI validation-tool installs with a documented update path.
