...
- `readmeDensity` records generated README line/details/table/category counts plus repo-only and low-signal row counts; density warnings are informational and feed the next portfolio-only review pass.
...
- Profile sync summaries include README density warnings, largest category, and repo-only row counts.
...
1. Decide whether density-warning rows should move to portfolio-only browsing, using `readmeDensity` evidence.
2. Add generated artifact/render-budget reporting if the density review keeps the README as a public routing surface.
3. Draft a PR-delivery transition checklist before any required-check enforcement is enabled.
