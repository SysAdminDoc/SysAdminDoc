...
- `.github/workflows/tests.yml` runs offline Pester plus pinned PSScriptAnalyzer on generator/setup changes.
...
- `profile-sync.yml` now runs on pull requests that touch profile contract paths, giving catalog/feed/schema/README changes an automatic generated-profile validation check before merge.
