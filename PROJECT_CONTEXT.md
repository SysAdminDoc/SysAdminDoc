...
- `profileReleaseConsistency` compares planning version `v4.9.63` against latest GitHub release `v3.0.0`, records the missing `v4.9.63` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
2. Add workflow-security trigger/audit coverage for future `.github/actions/**`.
