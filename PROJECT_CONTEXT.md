...
- `profileReleaseConsistency` compares planning version `v4.9.81` against latest GitHub release `v3.0.0`, records the missing `v4.9.81` tag ref, and surfaces 2 warning-only release/tag rows without failing `-Check`.
...
1. Draft a PR-delivery transition checklist before any required-check enforcement is enabled.
2. Use the density-routing report to choose concrete Python rows for portfolio-only catalog review if artifact budgets stay healthy.
3. Add a portfolio-only catalog review preview mode before changing generated README rows.
