...
| PR delivery or bypass | Needs live validation | Generated PR creation, branch-scoped workflow-dispatch validation, and the `generated-profile/validation` PR status handoff are proven. Routine maintenance has selected PR delivery with no direct-main bypass approved, and `routineMaintenancePrDrillEvidence` is pending. | Run a live routine maintenance PR merge drill, then record the PR/check/merge evidence before enabling required checks. |
