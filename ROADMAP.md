...
Current repo version: v4.9.124
Research baseline HEAD: `3d4ed8f Release v4.7.0 -- catalog refresh, drop private-repo refs`
P0 implementation baseline: `1fe3830 Consolidate profile research roadmap`

> Last researched: Cycle 132 - 2026-06-07.
...
2026-06-07 v4.9.124 refresh: Dependabot security posture recorded.
Cycle 132 adds `repositorySettings.security.dependabotSecurityPosture` so the
report distinguishes local Dependabot version-update coverage from the disabled
Dependabot security-updates repository setting. The new decision note
`docs/decisions/2026-06-07-dependabot-security-posture.md` records that local
`github-actions` and `npm` version-update config is present, but it does not
replace enabled Dependabot security updates. The repository warning remains
visible until security updates are enabled or a manual security-triage policy is
documented.
...
through the repository's allowed rebase merge path. All six candidate checks
passed: `Check generated README`, `PSScriptAnalyzer`, `Pester (offline)`,
`Markdownlint`, `Windows setup smoke`, and `zizmor`. The sync report now
records PR #14, head SHA `65475b7b47fc1e33a96843a131108b2660b18d19`,
merge SHA `64e02f3b4b9737f77b4629052dabc9f449e261bb`, workflow run IDs
`27090770215`, `27090770193`, and `27090770203`, `mergeMethod=rebase`, and
deleted-branch cleanup. Routine PR delivery is proven; required-check
enforcement remains disabled until branch protection or a repository ruleset is
selected, enabled, and re-queried.
...
refresh. GitHub's README documentation still notes that content beyond 500 KiB
is truncated when viewed on GitHub; the current generated README remains well
below that hard display threshold at about 61 KiB, with 587 lines, 110 table
rows, and 166 README project rows. The largest categories are `powershell` and
`python` at the 30-row soft cap, `readmeDensity.warningCount=0`,
`portfolioOnlyCandidateCount=0`, `routingRecommendation=keep-readme-routing-surface`,
and the report-only preview status is `no-candidates`. Pester now
guards those committed report properties so the next live metadata refresh will
fail locally if a new portfolio-only review queue appears.
...
Last autonomous roadmap pass: Cycle 132 - 2026-06-07.
...
- Cycle 121 fixed projects-feed check-only comparison so equivalent `pushedAt` timestamp serialization and volatile source-commit/snapshot provenance do not fail ordinary PR validation; stable provenance drift remains fatal.
...
- Branch-protection required-check enforcement is active. Routine maintenance uses pull-request delivery, no direct-main bypass is approved, PR #16 is the first recorded normal PR proof under active required checks, review requirements remain warning-only until an independent reviewer or team model exists, and Dependabot security updates remain warning-only until enabled or a manual triage policy is documented.

Next research cycles:

1. Cycle 133: audit userscript install-trust warnings and decide whether any branch-hosted install rows should move to release/tag-hosted URLs with explicit update metadata.
2. Cycle 134: audit branch-protection and workflow check-name stability after several routine PR merges under required checks.
3. Cycle 135: audit remaining community-health warnings for contributing/code-of-conduct guidance.
...
- [x] P2 — Dependabot security posture reporting (completed v4.9.124 with `dependabotSecurityPosture`, local config ecosystem evidence, summary rows, schema coverage, a decision note, and Pester guards).
...
- [x] P2 — Projects-feed check-only provenance normalization (completed v4.9.113 with `projects[].pushedAt` timestamp normalization, volatile source-commit/snapshot equality handling, regenerated feed/report, and Pester coverage that still catches stable provenance drift).
